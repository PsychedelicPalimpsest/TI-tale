...
export default function RoomExplorer() {
  ...
  }, [roomData, scale, showTiles, showInstances, showGrid]);
  ...
  const composite = useCallback(() => {
    ...
    if (autoGen) {
      ...
      applyOrderedDither(tctx, src.width, src.height);
    ...
    if (showViewport) {
      drawViewport(ctx, viewportX, viewportY, viewportW, viewportH, scale);
    }
  }, [viewportX, viewportY, viewportW, viewportH, showViewport, scale, autoGen, cacheVersion]);
  ...
  const clampViewport = useCallback((vx, vy) => {
    ...
    const maxX = Math.max(0, roomData.width - viewportW);
    const maxY = Math.max(0, roomData.height - viewportH);
    return {
      vx: Math.max(0, Math.min(Math.round(vx), maxX)),
      vy: Math.max(0, Math.min(Math.round(vy), maxY)),
    };
  }, [roomData, viewportW, viewportH]);
