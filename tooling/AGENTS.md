...
- **Drag-and-drop upload** --- The `AssetEditor` modal accepts OS file drops anywhere within it. A dark overlay with a "Drop PNG to upload" message appears on `dragenter`; the dropped file's basename (minus extension) seeds the upload label, falling back to `<W>x<H>` from the image dimensions. Non-image drops surface an inline error. The same upload path is shared with the "Upload redraw" file picker.
